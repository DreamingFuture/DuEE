...
def main():
    ...
    init_logger(log_file="./log/{}-{}.log".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), args.event_type))

    seed_everything(args.seed)

    args.output_model_path = os.path.join(args.output_dir, args.dataset, args.event_type, "best_model.pkl")
    args.output_model_metric_path = os.path.join(args.output_dir, args.dataset, args.event_type, "metric.json")
    # 设置保存目录
    ...
    for epoch, _ in enumerate(range(int(args.num_train_epochs))):
        ...
        if eval_f1 > best_f1:
            ...
            # 储存度量信息
            with open(args.output_model_metric_path, 'w', encoding='utf-8') as f:
                f.write(json.dumps({'eval_p': eval_p, 'eval_r': eval_r, 'eval_f1':eval_f1, 'eval_loss': eval_loss}))
